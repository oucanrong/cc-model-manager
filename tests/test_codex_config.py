...
class CodexConfigTests(unittest.TestCase):
    ...
    def test_each_provider_proxy_is_saved_and_reloaded_independently(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "config.json"
            ...
            claude_proxies = {
                provider: self._proxy_for_index(index)
                for index, provider in enumerate(PROVIDER_OPTIONS, start=1)
            }
            codex_proxies = {
                provider: self._proxy_for_index(index + 100)
                for index, provider in enumerate(CODEX_PROVIDER_OPTIONS, start=1)
            }
            for provider, proxy in claude_proxies.items():
                config.provider_settings[provider].proxy = proxy
            for provider, proxy in codex_proxies.items():
                config.codex.provider_settings[provider].proxy = proxy
            config.provider = PROVIDER_OPTIONS[0]
            config.proxy = claude_proxies[config.provider]
            manager.save(config)

            loaded = manager.load()

            for provider, proxy in claude_proxies.items():
                self.assertEqual(loaded.provider_settings[provider].proxy, proxy)
            for provider, proxy in codex_proxies.items():
                self.assertEqual(
                    loaded.codex.provider_settings[provider].proxy,
                    proxy,
                )
